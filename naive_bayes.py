...
def naiveBayes(train_set, train_labels, dev_set, smoothing_parameter, pos_prior):
    ...
    for document in dev_set:
        ...
        for word in document:
            ...
            positiveLikelihood += math.log((positiveCounter[word]+smoothing_parameter)/(num_total_words_positive+smoothing_parameter*num_uniquewords_positive))
            negativeLikelihood += math.log((negativeCounter[word]+smoothing_parameter)/(num_total_words_negative+smoothing_parameter*num_uniquewords_negative))
        positivePosterior = positiveLikelihood+ math.log(pos_prior)
        negativePosterior = negativeLikelihood+ math.log(1-pos_prior)
